    ...
    while interval < interval_count:
        for x, url in enumerate(prod_tracker_URLS):
            ...
            try:
                driver.get(url)
                try:
                    four_o_four = check_exists_by_xpath("//div[contains(@id, 'productList')]")
                    if(four_o_four is False):
                        ...
                        # Check stock
                        try:
                            stockList = driver.find_elements_by_xpath("//b[contains(@class, 'site-stock') and contains(@class, 'pull-right')]")
                            stockListText = []
