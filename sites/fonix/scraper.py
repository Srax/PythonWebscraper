    ...
    while interval < interval_count:
        for x, url in enumerate(prod_tracker_URLS):
            ...
            try:
                driver.get(url)
                try:
                    four_o_four = check_exists_by_xpath("//div[@class='not-found-page']")
                    if(four_o_four is False):
                        try:
                            title = driver.find_element_by_xpath("//div[@class='top-product-data']//h2").get_attribute("innerText").replace("\n", " - ")
                        except:
                            title = None
                        try:    
                        ...
                        # Check stock
                        try:
                            stock = driver.find_elements_by_xpath("//div[contains(@class, 'inner-stock-circles')]//div[contains(@class, 'store-stock-wrapper')]//span")
                            stockList = []
                            for elem in stock:
                                stockList.append(elem.get_attribute("textContent").replace("\xa0","").replace("\xa03", "").replace(".", "").replace("stk", "").replace("+", "").strip())
                            stockTextNumbers = ' '.join(stockList)
                            s = [int(s) for s in re.findall(r'-?\d+\.?\d*', stockTextNumbers)]
                            stock = round(sum(s)/2)
